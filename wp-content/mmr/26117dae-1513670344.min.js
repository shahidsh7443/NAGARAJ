...
Scroller=function(settings){var self=this;this.id=settings.id;this.body=$(document.body);this.window=$(window);this.element=$('#'+settings.id);this.wrapperClass=settings.wrapper_class;this.ready=!0;this.disabled=!1;this.page=1;this.offset=settings.offset;this.order=settings.order;this.throttle=!1;this.handle='<div id="infinite-handle"><span>'+text.replace('\\','')+'</span></div>';this.google_analytics=settings.google_analytics;this.history=settings.history;this.origURL=window.location.href;this.footer=$('#infinite-footer');this.footer.wrap=settings.footer;if(type=='scroll'){this.window.bind('scroll.infinity',function(){this.throttle=!0});self.gotop();setInterval(function(){if(this.throttle){this.throttle=!1;self.thefooter();self.refresh()}},300);self.ensureFilledViewport();this.body.bind('post-load',{self:self},self.checkViewportOnLoad)}else if(type=='click'){this.element.append(self.handle);this.element.delegate('#infinite-handle','click.infinity',function(){$('#infinite-handle').remove();self.refresh()})}};Scroller.prototype.check=function(){var bottom=this.window.scrollTop()+this.window.height(),threshold=this.element.offset().top+this.element.outerHeight(!1)-this.window.height();threshold=Math.round(threshold*0.75);return bottom>threshold};Scroller.prototype.render=function(response){this.body.addClass('infinity-success');this.element.append(response.html);this.body.trigger('post-load');this.ready=!0};Scroller.prototype.query=function(){return{page:this.page,order:this.order,scripts:window.infiniteScroll.settings.scripts,styles:window.infiniteScroll.settings.styles,query_args:window.infiniteScroll.settings.query_args,last_post_date:window.infiniteScroll.settings.last_post_date,}};Scroller.prototype.gotop=function(){var blog=$('#infinity-blog-title');blog.attr('title',totop);blog.bind('click',function(e){$('html, body').animate({scrollTop:0},'fast');e.preventDefault()})};Scroller.prototype.thefooter=function(){var self=this,width;if($.type(this.footer.wrap)==="string"){width=$('body #'+this.footer.wrap).outerWidth(!1);if(width>479)
...
return;this.ready=!1;loader='<span class="infinite-loader"></span>';this.element.append(loader);loader=this.element.find('.infinite-loader');color=loader.css('color');try{loader.spin('medium-left',color)}catch(error){}
query=$.extend({action:'infinite_scroll'},this.query());jqxhr=$.get(infiniteScroll.settings.ajaxurl,query);jqxhr.fail(function(){loader.hide();self.ready=!0});jqxhr.done(function(response){loader.hide();if(!response)
return;response=$.parseJSON(response);if(!response||!response.type)
return;if(response.type=='empty'){self.disabled=!0;self.body.addClass('infinity-end').removeClass('infinity-success')}else if(response.type=='success'){if(response.scripts){$(response.scripts).each(function(){window.infiniteScroll.settings.scripts.push(this.handle);if(this.extra_data){var data=document.createElement('script'),dataContent=document.createTextNode("//<![CDATA[ \n"+this.extra_data+"\n//]]>");data.type='text/javascript';data.appendChild(dataContent);document.getElementsByTagName(this.footer?'body':'head')[0].appendChild(data)}
...
self.element.append(self.handle);if(self.google_analytics&&'object'==typeof _gaq)
...
aveSetHeight=aveSetHeight/wrapperQty;else aveSetHeight=0;if(postsHeight<windowHeight&&(postsHeight+aveSetHeight<windowHeight)){self.ready=!0;self.refresh()}
...
pageNum=-1;else pageNum=setData.pageNum}
...
else{toggleState(jQuery(this),thisid,!1,!1)}
...
function is_valid_jquery_selector(sel){try{jQuery(sel);return!0}
catch(ex){return!1}}
...
else{toggleState(jQuery(this),id,!0,id)}
...
if(is_valid_jquery_selector('#'+anchor)&&jQuery('#'+anchor).length){if(!jQuery('#'+anchor).hasClass('colomat-close')){jQuery('#'+anchor).click()}
...
gallery.opened=!1}).on('transitionend.jp-carousel ','.jp-carousel-slide',function(e){if('transform'==e.originalEvent.propertyName){var transitionMultiplier=((Date.now()-transitionBegin)/1000)/e.originalEvent.elapsedTime;container.off('transitionend.jp-carousel');if(transitionMultiplier>=2)
$('.jp-carousel-transitions').removeClass('jp-carousel-transitions')}});$('.jp-carousel-wrap').touchwipe({wipeLeft:function(e){e.preventDefault();gallery.jp_carousel('next')},wipeRight:function(e){e.preventDefault();gallery.jp_carousel('previous')},preventDefaultEvents:!1});$('.jetpack-likes-widget-unloaded').each(function(){jetpackLikesWidgetQueue.push(this.id)});nextButton.add(previousButton).click(function(e){e.preventDefault();e.stopPropagation();if(nextButton.is(this)){gallery.jp_carousel('next')}else{gallery.jp_carousel('previous')}})}};var methods={testForData:function(gallery){gallery=$(gallery);if(!gallery.length||undefined==gallery.data('carousel-extra')){return!1}
...
selected=slides.eq(0);gallery.jp_carousel('selectSlide',selected,!1);return this},close:function(){$('body').css('overflow',originalOverflow);$('html').css('overflow',originalHOverflow);return container.trigger('jp_carousel.beforeClose').fadeOut('fast',function(){container.trigger('jp_carousel.afterClose')})},next:function(){var slide=gallery.jp_carousel('nextSlide');container.animate({scrollTop:0},'fast');if(slide){this.jp_carousel('selectSlide',slide)}},previous:function(){var slide=gallery.jp_carousel('prevSlide');container.animate({scrollTop:0},'fast');if(slide){this.jp_carousel('selectSlide',slide)}},resetButtons:function(current){if(current.data('liked'))
$('.jp-carousel-buttons a.jp-carousel-like').addClass('liked').text(jetpackCarouselStrings.unlike);else $('.jp-carousel-buttons a.jp-carousel-like').removeClass('liked').text(jetpackCarouselStrings.like)},selectedSlide:function(){return this.find('.selected')},setSlidePosition:function(x){transitionBegin=Date.now();return this.css({'-webkit-transform':'translate3d('+x+'px,0,0)','-moz-transform':'translate3d('+x+'px,0,0)','-ms-transform':'translate('+x+'px,0)','-o-transform':'translate('+x+'px,0)','transform':'translate3d('+x+'px,0,0)'})},selectSlide:function(slide,animate){var last=this.find('.selected').removeClass('selected'),slides=gallery.jp_carousel('slides').css({'position':'fixed'}),current=$(slide).addClass('selected').css({'position':'relative'}),attachmentId=current.data('attachment-id'),previous=gallery.jp_carousel('prevSlide'),next=gallery.jp_carousel('nextSlide'),width=$(window).width(),previous_previous=previous.prev(),next_next=next.next(),galleryWidth=gallery.width(),currentWidth=current.width(),left=Math.floor((galleryWidth-currentWidth)*0.5),info_left,method,animated,info_min;gallery.jp_carousel('loadFullImage',current);caption.hide();if(next.length==0&&slides.length<=2)
$('.jp-carousel-next-button').hide();else $('.jp-carousel-next-button').show();if(previous.length==0&&slides.length<=2)
$('.jp-carousel-previous-button').hide();else $('.jp-carousel-previous-button').show();method='css';animated=current.add(previous).add(previous_previous).add(next).add(next_next).jp_carousel('loadSlide');slides.not(animated).hide();current.jp_carousel('setSlidePosition',left).show();gallery.jp_carousel('fitInfo',animate);var direction=last.is(current.prevAll())?1:-1;if(1==direction){if(!next_next.is(previous))
next_next.jp_carousel('setSlidePosition',galleryWidth+next.width()).show();if(!previous_previous.is(next))
previous_previous.jp_carousel('setSlidePosition',-previous_previous.width()-currentWidth).show()}
else{if(!next_next.is(previous))
next_next.jp_carousel('setSlidePosition',galleryWidth+currentWidth).show()}
previous.jp_carousel('setSlidePosition',Math.floor(-previous.width()+(screenPadding*0.75))).show();next.jp_carousel('setSlidePosition',Math.ceil(galleryWidth-(screenPadding*0.75))).show();gallery.jp_carousel('resetButtons',current);container.trigger('jp_carousel.selectSlide',[current]);gallery.jp_carousel('getTitleDesc',{title:current.data('title'),desc:current.data('desc')});gallery.jp_carousel('loadLikes',attachmentId);gallery.jp_carousel('updateLikesWidgetVisibility',attachmentId)
if(next.length>0)
gallery.jp_carousel('loadLikes',next.data('attachment-id'));if(previous.length>0)
gallery.jp_carousel('loadLikes',previous.data('attachment-id'));var imageMeta=current.data('image-meta');gallery.jp_carousel('updateExif',imageMeta);gallery.jp_carousel('updateFullSizeLink',current);gallery.jp_carousel('updateMap',imageMeta);gallery.jp_carousel('testCommentsOpened',current.data('comments-opened'));gallery.jp_carousel('getComments',{'attachment_id':attachmentId,'offset':0,'clear':!0});$('#jp-carousel-comment-post-results').slideUp();if(current.data('caption')){if($('<div />').text(current.data('caption')).html()==$('<div />').text(current.data('title')).html())
...
$(next).add(previous).each(function(){gallery.jp_carousel('loadFullImage',$(this))});window.location.hash=last_known_location_hash='#jp-carousel-'+attachmentId},slides:function(){return this.find('.jp-carousel-slide')},slideDimensions:function(){return{width:$(window).width()-(screenPadding*2),height:Math.floor($(window).height()/100*proportion-60)}},loadSlide:function(){return this.each(function(){var slide=$(this);slide.find('img').one('load',function(){slide.jp_carousel('fitSlide',!1)})})},bestFit:function(){var max=gallery.jp_carousel('slideDimensions'),orig=this.jp_carousel('originalDimensions'),orig_ratio=orig.width/orig.height,w_ratio=1,h_ratio=1;if(orig.width>max.width)
w_ratio=max.width/orig.width;if(orig.height>max.height)
h_ratio=max.height/orig.height;if(w_ratio<h_ratio){width=max.width;height=Math.floor(width/orig_ratio)}else if(h_ratio<w_ratio){height=max.height;width=Math.floor(height*orig_ratio)}else{width=orig.width;height=orig.height}
return{width:width,height:height}},fitInfo:function(animated){var current=this.jp_carousel('selectedSlide'),size=current.jp_carousel('bestFit');photo_info.css({'left':Math.floor((info.width()-size.width)*0.5),'width':Math.floor(size.width)});return this},fitMeta:function(animated){var newInfoTop={top:Math.floor($(window).height()/100*proportion+5)+'px'};var newLeftWidth={width:(info.width()-(imageMeta.width()+80))+'px'};if(animated){info.animate(newInfoTop);leftColWrapper.animate(newLeftWidth)}else{info.animate(newInfoTop);leftColWrapper.css(newLeftWidth)}},fitSlide:function(animated){return this.each(function(){var selected=gallery.jp_carousel('selectedSlide'),$this=$(this),dimensions=$this.jp_carousel('bestFit'),method='css',max=gallery.jp_carousel('slideDimensions');dimensions.left=0;dimensions.top=Math.floor((max.height-dimensions.height)*0.5)+40;$this[method](dimensions)})},texturize:function(text){text=new String(text);text=text.replace(/'/g,'&#8217;').replace(/&#039;/g,'&#8217;').replace(/[\u2019]/g,'&#8217;');text=text.replace(/"/g,'&#8221;').replace(/&#034;/g,'&#8221;').replace(/&quot;/g,'&#8221;').replace(/[\u201D]/g,'&#8221;');text=text.replace(/([\w]+)=&#[\d]+;(.+?)&#[\d]+;/g,'$1="$2"');return $.trim(text)},initSlides:function(items,start_index){var width=this.jp_carousel('slideDimensions').width,x=0;if(items.length<2){$('.jp-carousel-next-button, .jp-carousel-previous-button').hide()}
else{$('.jp-carousel-next-button, .jp-carousel-previous-button').show()}
items.each(function(i){var src_item=$(this),orig_size=src_item.data('orig-size')||'',max=gallery.jp_carousel('slideDimensions'),parts=orig_size.split(',');orig_size={width:parseInt(parts[0],10),height:parseInt(parts[1],10)},medium_file=src_item.data('medium-file')||'',large_file=src_item.data('large-file')||'';src=src_item.data('orig-file');src=gallery.jp_carousel('selectBestImageSize',{orig_file:src,orig_width:orig_size.width,orig_height:orig_size.height,max_width:max.width,max_height:max.height,medium_file:medium_file,large_file:large_file});$(this).data('gallery-src',src)});if(0!==start_index)
...
slide.jp_carousel('fitSlide',!1)}});return this},selectBestImageSize:function(args){if('object'!=typeof args)
...
return Math.round(d)+'s';var df=1,top=1,bot=1;var limit=1e3;while(df!=d&&limit-->0){if(df<d){top+=1}
...
$('div#jp-carousel-comment-form-container').css('margin-top','20px');$('div#jp-carousel-comments-loading').css('margin-top','20px')},updateLikesWidgetVisibility:function(attachmentId){if("undefined"===typeof jetpackLikesWidgetQueue)
return;$('.jp-carousel-buttons .jetpack-likes-widget-wrapper').css('display','none').each(function(){var widgetWrapper=$(this);if(widgetWrapper.attr('data-attachment-id')==attachmentId){widgetWrapper.css('display','inline-block');return!1}})},loadLikes:function(attachmentId){var dataCarouselExtra=$('.jp-carousel-wrap').data('carousel-extra');var blogId=dataCarouselExtra.likes_blog_id;if($("#like-post-wrapper-"+blogId+"-"+attachmentId).length==0){var protocol='http';var originDomain='http://wordpress.com';if(dataCarouselExtra.permalink.length){var protocol=dataCarouselExtra.permalink.split(':')[0];if((protocol!='http')&&(protocol!='https'))
protocol='http';var parts=dataCarouselExtra.permalink.split('/');if(parts.length>=2)
originDomain=protocol+"://"+parts[2]}
var dataSource=protocol+"://widgets.wp.com/likes/#blog_id="+encodeURIComponent(blogId)+"&post_id="+encodeURIComponent(attachmentId)+"&slim=1&origin="+encodeURIComponent(originDomain);if('en'!==jetpackCarouselStrings.lang)
dataSource+="&lang="+encodeURIComponent(jetpackCarouselStrings.lang);var likesWidget=$("<iframe class='post-likes-widget jetpack-likes-widget jetpack-resizeable'></iframe>").attr("name","like-post-frame-"+blogId+"-"+attachmentId).attr('src',dataSource).css("display","inline-block");var likesWidgetWrapper=$("<div/>").addClass("jetpack-likes-widget-wrapper jetpack-likes-widget-unloaded slim-likes-widget").attr("id","like-post-wrapper-"+blogId+"-"+attachmentId).attr("data-src",dataSource).attr("data-name","like-post-frame-"+blogId+"-"+attachmentId).attr("data-attachment-id",attachmentId).css("display","none").css("vertical-align","middle").append(likesWidget).append("<div class='post-likes-widget-placeholder'></div>");$('.jp-carousel-buttons').append(likesWidgetWrapper)}},updateExif:function(meta){if(!meta||1!=jetpackCarouselStrings.display_exif)
return!1;var $ul=$("<ul class='jp-carousel-image-exif'></ul>");$.each(meta,function(key,val){if(0===parseFloat(val)||!val.length||-1===$.inArray(key,['camera','aperture','shutter_speed','focal_length']))
return;switch(key){case 'focal_length':val=val+'mm';break;case 'shutter_speed':val=gallery.jp_carousel('shutterSpeed',val);break;case 'aperture':val='f/'+val;break}
...
return;var latitude=meta.latitude,longitude=meta.longitude,$metabox=$('div.jp-carousel-image-meta','div.jp-carousel-wrap'),$mapbox=$('<div></div>'),style='&scale=2&style=feature:all|element:all|invert_lightness:true|hue:0x0077FF|saturation:-50|lightness:-5|gamma:0.91';$mapbox.addClass('jp-carousel-image-map').html('<img width="154" height="154" src="?\
						...
						<div class="gmap-topright"><div class="imgclip"><img width="175" height="154" src="?\
							...
							\
						').prependTo($metabox)},testCommentsOpened:function(opened){if(1==parseInt(opened,10)){$('.jp-carousel-buttons').fadeIn('fast');commentForm.fadeIn('fast')}else{$('.jp-carousel-buttons').fadeOut('fast');commentForm.fadeOut('fast')}},getComments:function(args){clearInterval(commentInterval);if('object'!=typeof args)
return;if('undefined'==typeof args.attachment_id||!args.attachment_id)
return;if(!args.offset||'undefined'==typeof args.offset||args.offset<1)
args.offset=0;var comments=$('.jp-carousel-comments'),commentsLoading=$('#jp-carousel-comments-loading').show();if(args.clear)
comments.hide().empty();$.ajax({type:'GET',url:jetpackCarouselStrings.ajaxurl,dataType:'json',data:{action:'get_attachment_comments',nonce:jetpackCarouselStrings.nonce,id:args.attachment_id,offset:args.offset},success:function(data,status,xhr){if(args.clear)
comments.fadeOut('fast').empty();$(data).each(function(){var comment=$('<div></div>').addClass('jp-carousel-comment').attr('id','jp-carousel-comment-'+this['id']).html('<div class="comment-gravatar">'+this['gravatar_markup']+'</div>'+'<div class="comment-author">'+this['author_markup']+'</div>'+'<div class="comment-date">'+this['date_gmt']+'</div>'+'<div class="comment-content">'+this['content']+'</div>');comments.append(comment);clearInterval(commentInterval);commentInterval=setInterval(function(){if(($('.jp-carousel-overlay').height()-150)<$('.jp-carousel-wrap').scrollTop()+$(window).height()){gallery.jp_carousel('getComments',{attachment_id:args.attachment_id,offset:args.offset+10,clear:!1});clearInterval(commentInterval)}},300)});var current=$('.jp-carousel div.selected');if(current&&current.data&&current.data('attachment-id')!=args.attachment_id){comments.fadeOut('fast');comments.empty();return}
...
commentTextArea.val('')},nextSlide:function(){var slides=this.jp_carousel('slides');var selected=this.jp_carousel('selectedSlide');if(selected.length==0||(slides.length>2&&selected.is(slides.last())))
return slides.first();return selected.next()},prevSlide:function(){var slides=this.jp_carousel('slides');var selected=this.jp_carousel('selectedSlide');if(selected.length==0||(slides.length>2&&selected.is(slides.first())))
return slides.last();return selected.prev()},loadFullImage:function(slide){var image=slide.find('img:first');if(!image.data('loaded')){image.on('load.jetpack',function(){image.off('load.jetpack');$(this).closest('.jp-carousel-slide').css('background-image','')});if(!slide.data('preview-image')||(slide.data('thumb-size')&&slide.width()>slide.data('thumb-size').width))
image.attr('src',image.closest('.jp-carousel-slide').data('src'));else image.attr('src',slide.data('preview-image'));image.data('loaded',1)}}};$.fn.jp_carousel=function(method){if(methods[method]){return methods[method].apply(this,Array.prototype.slice.call(arguments,1))}else if(typeof method==='object'||!method){return methods.open.apply(this,arguments)}else{$.error('Method '+method+' does not exist on jQuery.jp_carousel')}};$(document.body).on('click','div.gallery,div.tiled-gallery',function(e){if(!$(this).jp_carousel('testForData',e.currentTarget))
return;if($(e.target).parent().hasClass('gallery-caption'))
return;e.preventDefault();$(this).jp_carousel('open',{start_index:$(this).find('.gallery-item, .tiled-gallery-item').index($(e.target).parents('.gallery-item, .tiled-gallery-item'))})});$(window).on('hashchange',function(){if(!window.location.hash||!window.location.hash.match(/jp-carousel-(\d+)/))
return;if(window.location.hash==last_known_location_hash)
return;last_known_location_hash=window.location.hash;var gallery=$('div.gallery, div.tiled-gallery'),index=-1,n=window.location.hash.match(/jp-carousel-(\d+)/);if(!$(this).jp_carousel('testForData',gallery))
...
gallery.jp_carousel('open',{start_index:index})});if(window.location.hash)
$(window).trigger('hashchange')});(function($){$.fn.touchwipe=function(settings){var config={min_move_x:20,min_move_y:20,wipeLeft:function(e){},wipeRight:function(e){},wipeUp:function(e){},wipeDown:function(e){},preventDefaultEvents:!0};if(settings)$.extend(config,settings);this.each(function(){var startX;var startY;var isMoving=!1;function cancelTouch(){this.removeEventListener('touchmove',onTouchMove);startX=null;isMoving=!1}
